...
const makePropsSpy = typeof Proxy !== 'undefined'
  ? (obj, spy) => new Proxy(obj, {
...
// )
// ```
const computed = computed => Component => class extends PureComponent {
  constructor () {
    super()
        ...
        get: () => {
          ...
          if (!needsRecompute) {
            ...
            for (const depName in dependencies) {
              let value = (depName === name) || !(depName in cache)
                ? props[depName]
                : cache[depName]
